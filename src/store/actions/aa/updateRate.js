...
export const updateRate = () => async (dispatch, getState) => {
  ...
  const params = definitionActive && definitionActive[0].definition["1"].params;
  let data_feed;
  let data_feed_ma;
  try {
    ...
    if (
      ...
      for (let id in coins) {
        if (
          ...
          const exchange_rate = data_feed_ma;
          ...
          if (coins[id].collateral < min_collateral_liquidation) {
            ...
            const isEnded = moment(DateNow) > Number(coins[id].auction_end_ts);
